...
class CSS_QM_Output_Html_allpostmeta extends QM_Output_Html {
	...
	public function output() {
		...
		if (isset($wp_the_query) && is_object($wp_the_query->post) && count($wp_the_query->post))
			$post = $wp_the_query->post;
		else if (is_admin() && array_key_exists('post',$_GET))
		...
		if (count($meta))
			...
			echo '<tr><td colspan="3">No post meta.</td></tr>';
